...
- Сервер не создаёт и не добавляет компьютерных участников.
- Создатель комнаты — первый реальный игрок.
- Остальные участники могут войти только вручную по четырёхсимвольному коду комнаты.
- Список в лобби формируется только из записей, созданных запросами `create` и `join`.
